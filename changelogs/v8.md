...
3. Fixed an issue where the progress bar disappeared after the file download finished instead of showing the actual extraction progress.
4. Updated the NuGet package `Hardware.Info` to the latest version.
5. **NEW!** Added a new option `Keep benefit ZIP files as backup` for Stella Plus subscribers in the `Settings` window.
6. Improved and added tooltips in the `Settings` window.
7. Corrected the year from 2025 to 2026 in certain cases.
