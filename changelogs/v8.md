...
<aside class="warning" role="region" aria-label="ReShade">
  ...
  <p>
    This update allows quick switching between the standard version of ReShade and the version with full add-on and depth buffer support. The default setting is the standard version. For Stella Plus subscribers, the full add-on support version will be enabled automatically after launching 'Stella Plus App.exe'.
  </p>
</aside>
...
5. Set `webView21.Dock` to `Fill` in the Browser window.
6. Improved byte conversion logic and corrected the function responsible for displaying download speed.
7. Various quality improvements in Genshin Stella Mod.
8. Updated Turkish translations.
...
3. Updated the `Hardware.Info` and `NLog` libraries to the latest versions.
