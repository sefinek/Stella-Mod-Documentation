...
# 📥 Changelog for v8.8.0.0 from 12.09.2025 <!-- {#ver-880} -->
1. Migrated Stella Mod to .NET 9.
2. Stella Mod is now a self-contained application – it uses its own built-in .NET 9 runtime (hence the larger file size). This ensures consistent behavior across all systems.
...
7. Improved many English phrases to be smoother and more readable.
8. Improved the changelog window shown after each update – the previous one was crappy.
9. Fixed several WebSocket communication issues.
10. Minor security improvements.
11. Improved the overall user experience.
...
5. On the first launch, the application automatically sets the FPS limit based on the refresh rate of the primary monitor.
6. In the settings, the application displays a list of monitors along with their names, resolutions, and refresh rates.
7. Improved the layout in the settings window.
8. Improved the overall user experience.
9. Updated translations for Italian, Swedish, Russian.
10. Fixed an issue where the application might not minimize to the system tray.
