...
1. **NEW!** Added a new option `Keep benefit ZIP files as backup` for Stella Plus subscribers in the `Settings window`.
2. **NEW!** Implemented new API v9 (preview). This update introduces it on the client side.
3. Improved and added tooltips in the `Settings window`.
4. Improved the file download, extraction, and cleanup process.
5. Improved integration with `WebView2`. The component is now automatically disposed when launching the game, followed by a forced garbage collection to immediately free up RAM.
6. Increased the size of the changelog browser window.
7. Set `webView21.Dock` to `Fill` in the `Browser` window.
8. Fixed an issue where the progress bar disappeared after the file download finished instead of showing the actual extraction progress.
9. Fixed an issue where the `Status label: Update size: <>` and `Downloading file... <> / <>` labels displayed inconsistent values. Improved the byte conversion logic and corrected the function responsible for displaying download speed.
10. Fixed an issue where the YouTube player in the `Gameplay window` was throwing error 153.
11. Fixed broken links.
12. Corrected the year from `2025` to `2026` in certain cases.
13. Updated Windows Terminal to the latest version.
14. Updated the `Hardware.Info` and `NLog` libraries to the latest versions.
15. Updated Turkish translations.
