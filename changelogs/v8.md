...
# 📥 Changelog for v8.7.4.0 from 27.05.2025 <!-- {#ver-874} -->
1. Renamed `Stella.Telemetry` to `Stella.StartupBeacon`. Stella Mod does NOT send ANY telemetry data to the server, so the previous name could have misled users.
2. Renamed `Stella.Scripts` to `Stella.Utils`.
3. Fixed some issues in the `Tools` window that affected free users.
4. Stella Mod will no longer clean `%TEMP%` after updates.
5. Improved file deletion handling during each update.
6. Updated Turkish translations.
7. Other quality fixes.
